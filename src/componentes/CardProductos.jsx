...
const CardProductos = (props) => {
  ...
  const [isobservacion, setIsObservacion] = useState(false);
  ...
  const toggleIsObservacion = () => {
    setIsObservacion(!isobservacion)
  }

  const añadirObservacion = () => {
    setIsObservacion(false)
    Swal.fire({
      title: 'Go Pedidos',
      text: 'Observación añadida satisfactoriamente',
      icon: 'success',
  ...
  const confirmarProducto = () => {
    ...
    if (tallaSeleccionada === '') {
      ...
      Swal.fire({
        title: 'Go Pedidos',
        html: `<p>¿Añadir <b>${props.nombre}</b> a la cesta?</p>`,
        icon: 'question',
        showDenyButton: true,
        confirmButtonText: 'Si',
        confirmButtonColor: '#009b3e'
        ...
        if (respuesta.isConfirmed) {
          Swal.fire({
            title: 'Go Pedidos',
            html: `<p><b>${props.nombre}</b> ha sido añadido a la cesta</p>`,
            icon: 'success',
            confirmButtonColor: '#009b3e',
          })
        ...
        <div className='popup-bg-confirmarProducto'>
          <div className='popup-confirmarProducto'>
            ...
            <div className='agregar-confirmarProducto'>
              <p onClick={confirmarProducto}>Agregar a la cesta</p>
              <p onClick={toggleIsObservacion}>Añadir observación</p>
            </div>
            {isobservacion && (
              <div className='popup-bg-observacion'>
                <div className='popup-observacion'>
                  <img onClick={toggleIsObservacion} className="confirmarProducto-close-button-img" src={cerrar} alt="Cerrar" />
                  <p className='popup-observacion-titulo'>Añade una Observación</p>
                  <textarea
                    cols="30"
                    rows="5"
                    className='textareaProducto'
                    value={observacion}
                    onChange={(e) => setObservacion(e.target.value)}
                  >
                    {observacion}
                  </textarea>
                  <p onClick={añadirObservacion} className='boton botonAñadirObservacion'>Añadir</p>
                </div>
              </div>
            )}
          </div>
